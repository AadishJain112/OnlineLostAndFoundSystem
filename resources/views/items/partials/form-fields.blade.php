...
<div class="space-y-6">
    ...
    <div data-aos="fade-up">
        ...
        <textarea name="description" rows="4" class="input-float min-h-[120px] w-full resize-y"
            required>{{ old('description', $item?->description) }}</textarea>
        <x-input-error :messages="$errors->get('description')" class="mt-2" />
    </div>

    <div x-data="{ preference: '{{ old('contact_preference', $item?->contact_preference?->value ?? 'platform') }}' }"
        class="grid gap-6 md:grid-cols-2" data-aos="fade-up">
        <div>
            ...
            <input type="date" name="{{ $dateField }}"
                value="{{ old($dateField, optional($item?->{$dateField})->format('Y-m-d')) }}"
                class="input-float w-full !py-3" required>
            <x-input-error :messages="$errors->get($dateField)" class="mt-2" />
        </div>
        <div>
            <label class="mb-2 block text-sm font-semibold text-slate-700 dark:text-slate-300">Contact
                preference</label>
            <select name="contact_preference" x-model="preference" class="input-float w-full !py-3">
        ...
        <div x-show="preference === 'email'" x-cloak class="md:col-span-2">
            <x-ui.input-floating label="Contact email" name="contact_email" type="email" :value="old('contact_email', $item?->contact_email)" />
            <x-input-error :messages="$errors->get('contact_email')" class="mt-2" />
        </div>
    </div>

    <div data-aos="fade-up">
        <x-ui.input-floating label="Location" name="location" :value="old('location', $item?->location)" required
            data-location-input />
        ...
        <div id="map-picker" data-map-picker
            class="mt-3 flex h-48 cursor-crosshair items-center justify-center rounded-2xl border-2 border-dashed border-brand-500/30 bg-gradient-to-br from-brand-500/5 to-violet-500/5 text-sm text-slate-500 transition-all hover:border-brand-500/50">
